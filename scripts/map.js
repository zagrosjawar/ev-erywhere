...
function calculateRoute() {
  ...
  const waypoints = Array.from(document.querySelectorAll(".stops-input")).map(
    (input) => ({
      location: input.value,
      stopover: true,
    })
...
function updateNavigationPane(route) {
  ...
  document.getElementById("stationTimeDistance").textContent =
    "15 mins / 10 km";
  document
    .getElementById("stationStatsLink")
    .setAttribute("href", "statistikk.html");
}
...
function toggleNavigationPane() {
  const pane = document.getElementById("navigationPane");
  const stickyPane = document.getElementById("sticky-part");
  ...
  if (pane.classList.contains("active")) {
    ...
    pane.classList.add("active");
    console.log("passed first else");
  }
  console.log("passed first loop");
  // Check if the sticky is active (fully visible)
  if (stickyPane.classList.contains("active")) {
    // Move sticky to only show the top 10% (adjust as needed)
    stickyPane.classList.remove("active");
    console.log("removed class from sticky");
  } else {
    // Move sticky to fully visible
    stickyPane.classList.add("active");
    console.log("added class to sticky");
  }
}
